  ...
  return (
    <div className="hud-stage" onMouseDown={startDrag}>
      <div
        ...
        style={stopping ? { opacity: 0.6, pointerEvents: "none" } : undefined}
      >
        <button className="rec-stop" onClick={onStop} title="Stop recording" disabled={stopping}>
          <HudIco.stop size={20} />
          <span className="rec-time">
            {stopping ? "Saving…" : fmt(Math.max(0, elapsedMs))}
          </span>
        </button>
        ...
        <button className="rec-btn" onClick={onRestart} title="Restart recording" disabled={stopping}>
          <HudIco.restart size={18} />
        </button>
        <button className="rec-btn" onClick={onCancel} title="Discard recording" disabled={stopping}>
